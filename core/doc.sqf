...
player createDiaryRecord ["Documentation", ["Butts Changes", "
1) Changed Huey to Blackhawk.<br/>2) Moved base to AAC Airfield, added lights and shit.<br/>3) Added Engineer role<br/>4) Added Boat.<br/>5) Ensured most vehicles can be lifted or towed.<br/>6) Added Medical Facility, though probably only useful for Advanced Medical.<br/>7) Updated Bonus and Malus values (respawn, vehicle killed, hurt/heal civilian).<br/>8) firenear script (take rep hit when shooting near civilians) changed to (I believe) if enemy is not within 500 meters of a civilian, and you are within 50 meters of a civilian, rep goes down.<br/>9) Replaced AH64 with AH-9 variant.<br/>10) A towing HEMMTT has been added that can tow the Bradley.<br/>11) Updated some default Params that fit with our gameplay.<br/>12) wreck script updated so should repair blackhawk now.<br/>13) Arsenal added to FOB flags (FOB's may also be full repair facilities but untested).
	"]
];